...
function installTradefedContentProvider() {
    ...
    exec(`${adb} -s ${questIpAddress}:5555 install --user ${questUserId} -g TradefedContentProvider.apk`, (error, stdout, stderr) => {
        ...
        console.log(`- Success, we can now accept !bsr request`);
    });
}
...
function pushMapToQuest(hash) {
    ...
    const dir = fs.opendirSync('123')
    let dirent
    while ((dirent = dir.readSync()) !== null) {
        exec(`${adb} -s ${questIpAddress}:5555 shell content write --user ${questUserId} --uri content://android.tradefed.contentprovider/sdcard/ModData/com.beatgames.beatsaber/Mods/SongLoader/CustomLevels/${hash}/dirent.name < tmp/${hash}/dirent.name`, (error, stdout, stderr) => {
